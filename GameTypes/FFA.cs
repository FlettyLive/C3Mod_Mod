		...
		public static void OnUpdate(EventArgs args)
		{
			lock (C3Mod.C3Players)
			{
				if (C3Mod.VoteRunning && C3Mod.VoteType == "ffa")
				{
					...
					{
						C3Tools.BroadcastMessageToGametype("ffa", "Vote to play Free For All passed! Teleporting to start positions!", Color.DarkCyan);
				...
				if (FFARunning)
				{
						...
						foreach (C3Player player in C3Mod.C3Players)
						{
							...
							if (player.GameType == "ffa")
							{
								...
								if (!Main.player[player.Index].dead && player.Dead)
								{
									...
									C3Tools.BroadcastMessageToGametype("ffa", player.TSPlayer.Name + " died!", Color.Red);
									if (C3Mod.C3Config.TPLockEnabled)
									{
										player.TSPlayer.TpLock = true;
									}
								...
								{
									FFARunning = false;
									C3Tools.BroadcastMessageToGametype("ffa", player.TSPlayer.Name + " won!", Color.LightBlue);
					...
					if (players == 1)
					{
						C3Tools.BroadcastMessageToGametype("ffa", "Ending game, because not enough players to continue!", Color.DarkCyan);
						FFARunning = false;
						TpToSpawns(false);
						C3Tools.ResetGameType("ffa");
						TDM.TDMSpawns = new Vector2[2];
